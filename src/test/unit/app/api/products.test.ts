...
describe('Products API Logic', () => {
  ...
  describe('バリデーション処理', () => {
    it('商品名バリデーションが動作する', () => {
      const validateProductName = (name: string) => {
        ...
        if (name.length > 100) {
          return { valid: false, error: '商品名は100文字以内で入力してください' }
        }
        return { valid: true }
      }
    ...
    it('価格バリデーションが動作する', () => {
      const validatePrice = (price: number) => {
        ...
        if (price > 9999999) {
          return { valid: false, error: '価格は9,999,999円以下で入力してください' }
        }
        return { valid: true }
      }
  ...
  describe('エラーハンドリング', () => {
    ...
    it('成功レスポンスが正しい形式で生成される', () => {
      ...
      const successResponse = createSuccessResponse(mockProduct, '商品を取得しました')
